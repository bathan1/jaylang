...
(** Checks if key X is in the list of assignments A and if so, what boolean
    value_tuple0 it takes on. If X is not in A, then this returns `None` *)
let rec lookup (equal : 'a -> 'a -> bool) (x : 'a) (a : 'a assignment) :
    bool option =
  match a with
  | [] -> None
...
let eval_literal (equal : 'a -> 'a -> bool) (a : 'a assignment)
    (lit : 'a literal) : bool option =
  match lit with
...
*)
let simplify_clause (equal : 'a -> 'a -> bool) (a : 'a assignment)
    (c : 'a clause) : [ `Satisfied | `Conflict | `Reduced of 'a clause ] =
...
(** Prunes one level of literals from the clauses in CNF based on assignments in
    A. *)
...
(** Repeatedly applies unit propagation on CNF, extending the assignment A and
    simplifying clauses until no units remain or a conflict is found. *)
let rec unit_propagate equal a cnf =
  let cnf', conflict = simplify_cnf equal a cnf in
  if conflict
  then (a, cnf', true)
  else
    match
      List.find cnf' ~f:(fun c ->
    ...
    | Some clause ->
        let lit =
          List.find_exn clause ~f:(fun l ->
              match l with Pos x | Neg x -> Option.is_none (lookup equal x a))
        in
        let v, value =
          match lit with Pos x -> (x, true) | Neg x -> (x, false)
        in
        let a' = (v, value) :: a in
        unit_propagate equal a' cnf'

(** Checks if CNF with current assignments A is satisfiable or not. *)
let rec dpll (equal : 'a -> 'a -> bool) (a : 'a assignment) (cnf : 'a cnf) :
    'a assignment option =
  let a, cnf, conflict = unit_propagate equal a cnf in
  if conflict || List.exists cnf ~f:List.is_empty
  then None
  else if List.is_empty cnf
  then Some a
  else
    let vars =
      cnf |> List.concat
...
let simplify equal (cnf : 'a cnf) : 'a assignment * 'a cnf =
  let rec loop a cnf =
    let cnf', conflict = simplify_cnf equal a cnf in
    if conflict
    then (a, []) (* or raise, depending on design *)
    else
      match
        List.find cnf' ~f:(fun c ->
            List.count c ~f:(fun lit ->
