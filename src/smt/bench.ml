...
let () =
    ...
    let b_is_even =
        Smt.Formula.Binop(
            Equal,
            Binop (Modulus, Key b, Const_int 2),
            Const_int 0
        )
    in

    let a_gte_5 =
        Smt.Formula.Binop(Less_than_eq, Key a, Const_int 5)
    in

    let b_lt_a =
        Smt.Formula.Binop(Less_than, Key b, Key a)
    in
    ...
    let b_gt_a_minus_3 =
        Smt.Formula.Binop(
            Greater_than,
            Key b,
            Smt.Formula.Binop(Minus, Key a, Const_int 3)
        )
    in
    ...
    (* --- Solve with IDL first, fallback to Typed Z3 --- *)
